...
Read database.py to understand the expenses table schema , the db connection pattern , and the database file name.
...
Before generatingt anything, confirm the user_id exists in the users table. If not stop and say: "No user found with id <user_id>"

##Step-3 : Generate and insert expenses

Write and run a Python script that:
1. Spreads expenses randomly across the past <months> months 
2. Uses these categories with realistic Indian description
and amounts (pkr):
Food: 50-800
Transport: 20-500
Bills: 200-3000
...
5. Uses the parameterized queries only - no string formatting in Sql 
